...
if $kraken; then
	...
	for j in $workingFolder/trimmed*fastq.gz; do
		k=$(basename $j)
		conda run -n sispa kraken2 --db /home/cwduffy/kraken_db/virus/ --use-names --threads 56 --report $workingFolder/${k/.fastq.gz/}.kraken.report.txt --output $workingFolder/${k/.fastq.gz/}.kraken $j
		conda run -n sispa bracken -d /home/cwduffy/kraken_db/virus/ -i $workingFolder/${k/.fastq.gz}.kraken.report.txt -o $workingFolder/${k/.fastq.gz/}.bracken -w $workingFolder/${k/.fastq.gz/}.bracken.report
	done
...
if [[ -v consensus ]]; then
	...
	for j in $workingFolder/trimmed*fastq.gz; do
		...
		conda run -n sispa minimap2 $workingFolder/$refIndex.mmi -at 56 $j | conda run -n sispa samtools view -bT $consensus -@ 56 -o $workingFolder/$k.bam
		conda run -n sispa samtools sort -@ 56 $workingFolder/$k.bam -o $workingFolder/$k.sorted.bam
		rm -rf $workingFolder/$k.bam
		conda run -n sispa bcftools mpileup -Ou -f $consensus $workingFolder/$k.sorted.bam --threads 56 --annotate FORMAT/AD,INFO/AD | conda run -n sispa bcftools call -mv -Oz --ploidy 2 --threads 56 > $workingFolder/$k.vcf.gz
