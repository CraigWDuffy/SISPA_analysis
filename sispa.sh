...
if $kraken; then
	...
	for j in $workingFolder/trimmed*fastq.gz; do
		k=$(basename $j)
		conda run -n sispa kraken2 --db /home/cwduffy/kraken_db/virus/ --use-names --threads 56 --report $workingFolder/${k/.fastq.gz/}.report.txt --output $workingFolder/${k/.fastq.gz/}.kraken $j
		conda run -n sispa bracken -d /home/cwduffy/kraken_db/virus/ -i $workingFolder/${k/.fastq.gz}.report.txt -o $workingFolder/${k/.fastq.gz/}.bracken -w $workingFolder/${k/.fastq.gz/}.bracken.report
	done
